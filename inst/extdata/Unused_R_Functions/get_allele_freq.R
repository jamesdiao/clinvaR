...
#'
#' @usage get_allele_freq(input, ind, dataset, method)
...
#' @examples 
#' freq_gnomad.calc.gene <- get_allele_freq()
#' freq_1000g.count.gene <- get_allele_freq(input = merged_1000g, ind = F, dataset = "1000G", method = "Gene")
#' freq_exac.calc.gene <- get_allele_freq(input = merged_exac, ind = T, dataset = "EXAC",method = "Gene")
#' #@export

get_allele_freq <- function(input, ind, dataset, method) {
  #if (!("CLNSIG" %in% colnames(input)))
  #  input$CLNSIG <- rep(5, nrow(input))
  if (missing(dataset)) dataset <- '1000G'
             ...
             sapply(1:nrow(ACMG_Lit_Full), function(index) {
               ...
               sapply(item.vec.split, function(item) {
                 ...
                 sapply(c(dataset,super.levels), function(superpop) {
                   if (ind) {
                     return(aggregate_calc(input, superpop, item, dataset, loc, inherit.use))
                   } else {
                     return(aggregate_count(input, superpop, item, dataset, loc, inherit.use))
                   }
                 })
