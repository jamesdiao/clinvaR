...
over_time <- function(vcf, dates, verbose, plot.it, prevalence, case_freq) {
  ...
  output <- lapply(as.character(dates), function(dat) {
    ...
    return(list(freqs, positions, assertions) %>% 
             setNames(c('Freqs','Positions','Assertions')))
  ...
  if (!missing(prevalence) & !missing(case_freq)) {
    ...
    return(list(freqs, submissions, penetrance) %>% 
             setNames(c('Frequencies','Submissions','Penetrance')))
  }
  if (plot.it) {
    plot(freqs)
    plot(submissions)
  }
  return(list(freqs, submissions) %>% setNames(c('Frequencies','Submissions')))
}
