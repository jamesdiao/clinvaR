...
hcm_vcf <- import_file_1000g(genes = hcm_panel)
```
...
The ClinVar VCF can be used to annotate the Gene-Variant VCF.  

```{r, cache = T}
hcm_vcf <- annotate_1000g(vcf = hcm_vcf, clinvar = newest_clinvar)
hcm_vcf[1:5,1:5]
```

If we wanted to do this faster (with all the defaults used above), we could have done this more quickly by simply specifying the genes in annotate. We can also specify whether we want to exclude the (likely) pathogenic variants with any (likely) benign conflicts as well. These are included by default. 

```{r, cache = T}
hcm_vcf <- annotate_1000g(genes = hcm_panel, conflicts = FALSE)
hcm_vcf[1:5,1:5]
```
...
The same plots can be generated for the un-annotated VCF using a slightly different plot function (plot.plain_vcf). 
```{r}
plot(vcf)
```
...
```{r}
submissions_over_time(genes = hcm_panel) -> submits
submits
```
